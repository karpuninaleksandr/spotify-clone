                ...
                <div className="genres_container">
                    {genres ? 
                        (
                            genres.map(
                                (genre) => (
                                    <button className="genre" key={genre.id} onClick={
                                        (e) => 
                                            API.get_playlists(genre.id).then(
                                                (playlists) => {
                            ...
                            <>
                                <p className="empty_alert">Unfortunately, there was a problem connecting to API :(</p>
                                <p className="empty_alert">Please, try reloading page</p>
                            </>
                        )
                    }
                </div>
                ...
                <div className="albums_container">
                    {playlists ? 
                        (
                            playlists.map(
                                (playlist) => (
                                    <button className="album" key={playlist.id} onClick={
                                        (e) => 
                                            API.get_tracks(playlist.id).then(
                                                (tracks) => {
                                                    setTracks(tracks)
                                                }
                                            )
                            ...
                            <>
                                <p className="empty_alert">Unfortunately, no albums were found for this genre :(</p>
                                <p className="empty_alert">Please, try checking other genres</p>
                            </>
                        )
                    }
                </div>
                ...
                <div className="tracks_container">
                    {tracks ? 
                        (
                            tracks.map(
                                (track) => (
                                    <button className="track" key={track.track.id} onClick={
                                        (e) =>
                                            API.get_track(track.track.id).then(
                                                (track) => {
                                                    setTrack(track)
                                                }
                                            )
                            ...
                            <>
                                <p className="empty_alert">Unfortunately, no tracks were found for this album :(</p>
                                <p className="empty_alert">Please, try checking other albums</p>
                            </>
                        )
                    }
                </div>
